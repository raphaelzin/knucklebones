...
import { appendRoomState, getRoomState } from "../../redis/GameStateAdapter"
...
export class GameRoom {
    ...
    constructor(code: string) {
        ...
        // client.subscribe(`room:${code}`, message => {
        //   console.log(`Room ${code} got the message: `, JSON.stringify(message));
        // });
    ...
    handleGameStateChange(state: GameStateSummary) {
        for (const client of [...this.players, ...this.spectators]) {
            if (client.socket) this.emit(client.socket, { kind: "game-state-update", state })
    ...
    setupPubSubs() {
        ...
        client.subscribe(`room:${this.code}`, message => {
